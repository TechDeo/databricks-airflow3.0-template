# Create logs directory for persistent volume
mkdir -p tmp/logs

# Create or replace a kind cluster
kind delete cluster --name kind
kind create cluster --image kindest/node:v1.29.4 --config k8s/secrets/clusters/kind-cluster.yaml
...
# NOTE: TLS skip is configured in kind cluster config, so images will be pulled directly by the cluster
